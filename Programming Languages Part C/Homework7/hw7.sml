...
(* helper function for the shift operation
   shifts all values (non-values will first be evaluated and then passed into the function) *)
fun shift(dx, dy, exp) =
	case exp of
	NoPoints => NoPoints
...
(* preprocess expression before interpreting it*)
		...
		if real_close(x1, x2) andalso real_close(y1, y2) then 
			...
			if (x1 > x2) then
				if (y1 > y2) then
					...
					LineSegment(x2, y1, x1, y2)
			else
				if (y1 > y2) then
					LineSegment(x1, y2, x2, y1)
				else
					LineSegment(x1, y1, x2, y2)
	| Var _ => e
