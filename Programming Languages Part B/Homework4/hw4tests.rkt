...
(define tests
  (test-suite
   ...
   ;sequence test
   (check-equal? (sequence 0 5 1) '(0 1 2 3 4 5) "Sequence test 1")

   (check-equal? (sequence 0 0 1) '(0) "Sequence test 2")

   (check-equal? (sequence 3 11 2) '(3 5 7 9 11) "Sequence test 3")

   (check-equal? (sequence 3 8 3) '(3 6) "Sequence test 4")

   (check-equal? (sequence 3 2 1) '() "Sequence test 5")
   ...
   (check-equal? (string-append-map 
                  '() 
                  ".jpg") '() "string-append-map test 2")

   (check-equal? (string-append-map 
                  (list "dan" "dog" "curry" "dog2") 
                  "") '("dan" "dog" "curry" "dog2") "string-append-map test 3")

   (check-equal? (string-append-map 
                  (list "" "" "" "") 
                  ".jpg") '(".jpg" ".jpg" ".jpg" ".jpg") "string-append-map test 1")
            
   
   ;list-nth-mod test
   (check-equal? (list-nth-mod (list 0 1 2 3 4) 2) 2 "list-nth-mod test 1")

   (check-equal? (list-nth-mod (list 0 1 2 3 4) 100) 0 "list-nth-mod test 2")
   ...
   (check-equal? (stream-for-n-steps ones 5) (list 1 1 1 1 1) "stream-for-n-steps test")
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero dan-then-dog) 5) (list (cons 0 "dan.jpg") (cons 0 "dog.jpg") (cons 0 "dan.jpg") (cons 0 "dog.jpg") (cons 0 "dan.jpg")) "stream-add-zero test")

   ;cycle-lists test
   (check-equal? (stream-for-n-steps (cycle-lists (list 1 2 3) (list "a" "b")) 3) (list (cons 1 "a") (cons 2 "b") (cons 3 "a")) 
                 "cycle-lists test")

   (check-equal? (stream-for-n-steps (cycle-lists (list 1) (list "a")) 5) (list (cons 1 "a") (cons 1 "a") (cons 1 "a") (cons 1 "a") (cons 1 "a")) 
                 "cycle-lists test")
   ...
   (check-equal? (vector-assoc 4 (vector (cons 2 1) 3 4 5)) #f "vector-assoc test")

   (check-equal? (vector-assoc 10 (vector (cons 2 1) (cons 3 1) (cons 4 1) (cons 5 1))) #f "vector-assoc test")

   (check-equal? (vector-assoc 5 (vector (cons 2 1) (cons 3 1) (cons 4 1) (cons 5 1))) (cons 5 1) "vector-assoc test")

   (check-equal? (vector-assoc 2 (vector)) #f "vector-assoc test")
   ;   
   ;   ; cached-assoc tests
   
   (check-equal? ((cached-assoc (list (cons 1 2) (cons 3 4) (cons 3 5)) 3) 3) (cons 3 4) "cached-assoc test")
   ;   
   ;   ; while-less test
